...
直接输入 `/cursor` 命令，插件立刻调用 Cursor CLI，结果**原样返回，不经过 LLM 二次总结**。
...
你正常聊天，OpenClaw 的 AI 自动判断"这个问题需要看代码"，然后**主动调用** `cursor_agent` 工具。
...
通过 cursor-agent 调用 Cursor CLI 时，这些 MCP 配置**自动生效**。也就是说，你用手机远程排查问题时，AI 不只是看代码——它还能直接查数据库、看监控、翻 GitLab，拿到和你坐在电脑前一样的完整上下文。

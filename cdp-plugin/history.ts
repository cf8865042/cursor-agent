...
import { CDP_PORT, connectTarget, evaluate, click, pressEscape, sleep, type WindowKind } from "./cdp-utils.js";

async function readAgentHistory(ws: import("ws").default, keyword: string) {
  ...
  `, 1)) as Array<{ idx: number; group: string; name: string }>;
  return result;
}

async function readEditorHistory(ws: import("ws").default, keyword: string) {
  const hasKeyword = keyword && keyword !== "undefined";
  let msgId = 1;
  ...
  `, msgId++)) as Array<{ idx: number; group: string; name: string }>;
...
export const historyCommand = cli({
  ...
  func: async (_page: unknown, args: Record<string, unknown>) => {
    ...
    try {
      ...
      return [{ idx: 0, group: "ERROR", name: (e as Error).message }];
    }

    try {
      const sessions = kind === "agent"
        ? await readAgentHistory(ws, keyword)
        : await readEditorHistory(ws, keyword);

      const hasKeyword = keyword && keyword !== "undefined";
      return sessions && sessions.length > 0
        ? sessions
        : [{ idx: 0, group: "-", name: hasKeyword ? `No sessions matching "${keyword}"` : "No history sessions" }];
    } finally {
      ws.close();
    }
  },

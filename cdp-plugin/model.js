...
const modelCommand = cli({
  ...
  func: async (_page, args) => {
    ...
    try {
      ...
      return [{ idx: 0, name: "ERROR", tier: "", current: e.message }];
    }
    let msgId = 1;
    try {
      ...
      return (models || []).map((m) => {
        const isMatch = currentModel && (currentModel === m.name || currentModel === `${m.name}  ${m.tier}`.trim() || currentModel === `${m.name} ${m.tier}`.trim() || currentModel.replace(/\s+/g, " ") === `${m.name} ${m.tier}`.trim());
...
const modelSwitchCommand = cli({
  ...
  func: async (_page, args) => {
    ...
    try {
      ...
      return [{ status: "ERROR", model: e.message }];
    }
    let msgId = 1;
    try {
      const btnPos = await evaluate(ws, `
